...
export async function GET(request: NextRequest) {
  ...
  const lookbackHoursRaw = Number(
    searchParams.get('lookbackHours') ||
      process.env.MATCHING_AGENT_CRON_LOOKBACK_HOURS ||
      24
  );
  const lookbackHours = Number.isFinite(lookbackHoursRaw)
    ? Math.max(1, Math.min(168, Math.floor(lookbackHoursRaw)))
    : 24;
  ...
  try {
    ...
    if (error) {
      throw new Error(`Failed to load jobs for matching cron: ${error.message}`);
    }
